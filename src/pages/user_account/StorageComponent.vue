<template>
                ...
                <q-tab-panels v-model="selectedTab" animated swipeable vertical transition-prev="jump-up"
                    ...
                    <q-tab-panel v-for="tab in tabs" :key="tab.name" :name="tab.name">
                        <common-table :title="tab.name" description="" :columns="tableColumns" :rows="tab.tableData"
                            :searchInputPlaceholder="searchLabel" :showAddButton="false" :hideBottom="false"
                            noDataLabel="No Data" @add-new="openTableDialog" @move-row="openMoveDialog"
                            @rename-row="openRenameDialog" showStorage isMultipleChecked />

                        <template v-if="tab.tableData.length <= 0">
                            <div class="row justify-center q-pt-lg">
                                <div class="column q-pa-lg bg-custom-primary text-center bucket-file-upload">
                                    <div class="flex flex-center q-mb-md">
                                    ...
                                    <q-btn unelevated label="Upload A File" icon="add" :ripple="false"
                                        class="bg-light-green rounded-10 text-white text-capitalize self-center btn-add-new q-mt-sm"
                                        @click="addNewBucketDialog" />
                                </div>
                            </div>
    ...
    <q-dialog v-model="isMovingRow" backdrop-filter="blur(4px)" class="storage-dialog">
        <q-card class="highlighted-border">
            ...
            <q-card-section class="q-pb-lg">
                ...
                <q-input dense outlined placeholder="e.g folder1/subfodler"
                    class="rounded-10 self-center text-weight-light rounded-input bg-custom-primary" />
                <p class="text-custom-gray-dark text-weight-light q-pt-xs">Leave blank to move items to the root of the
                    bucket</p>
            </q-card-section>
            <q-separator />
            <q-card-actions align="right" class="q-pa-md">
                <q-btn label="Cancel" unelevated color="dark"
                    class="rounded-10 text-capitalize min-w-80 highlighted-border" @click="isMovingRow = false" />
    ...
    <q-dialog v-model="isRenameRow" backdrop-filter="blur(4px)" class="storage-dialog">
        <q-card class="highlighted-border">
            ...
            <q-card-section class="q-pb-lg">
                ...
                <q-input dense outlined placeholder="e.g my-image"
                    class="rounded-10 self-center text-weight-light rounded-input bg-custom-primary" />
            </q-card-section>
            <q-separator />
            <q-card-actions align="right" class="q-pa-md">
                <q-btn label="Cancel" unelevated color="dark"
                    class="rounded-10 text-capitalize min-w-80 highlighted-border" @click="isRenameRow = false" />
    ...
    <q-dialog v-model="addNewBucket" backdrop-filter="blur(4px)" class="snippet-dialog">
        <q-card class="highlighted-border">
            <q-card-section class="flex justify-between items-center q-pa-lg">
                <div class="flex items-center q-gutter-sm">
                ...
                <q-icon name="close" class="cursor-pointer fs-20" @click="addNewBucket = false" />
            </q-card-section>
            <q-separator />
            <q-card-section class="q-pb-lg">
                ...
                <q-input dense outlined placeholder="e.g my-bucket"
                    class="rounded-10 self-center text-weight-light rounded-input bg-custom-primary" />
            </q-card-section>
            <q-separator />
            <q-card-actions align="right" class="q-pa-md">
                <q-btn label="Cancel" unelevated color="dark"
                    class="rounded-10 text-capitalize min-w-80 highlighted-border" @click="addNewBucket = false" />
